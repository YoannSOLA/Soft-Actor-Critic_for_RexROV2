...
class DPControllerBaseRL(object):
    ...
    def __init__(self, is_model_based=False, list_odometry_callbacks=[],
        ...
        self._services['set_pid_mode'] = rospy.Service('set_pid_mode',
                                                SetPIDMode,
                                                self.set_pid_mode_callback)
    ...
    def get_controller(name, *args):
        ...
        for controller in DPControllerBaseRL.__subclasses__():
    ...
    def get_list_of_controllers():
        ...
        return [controller.__name__ for controller in
                DPControllerBaseRL.__subclasses__()]
    ...
    def set_pid_mode_callback(self, request):
        self.pid_mode = request.pid_mode
        return SetPIDModeResponse(True)
    ...
    def _odometry_callback(self, msg):
        ...
        msg.pose.pose.position.x = np.squeeze(msg.pose.pose.position.x + np.random.normal(0, random.uniform(.1, .05), 1))
        msg.pose.pose.position.y = np.squeeze(msg.pose.pose.position.y + np.random.normal(0, random.uniform(.1, .05), 1))
        msg.pose.pose.position.z = np.squeeze(msg.pose.pose.position.z + np.random.normal(0, random.uniform(.1, .05), 1))

        msg.pose.pose.orientation.x = np.squeeze(msg.pose.pose.orientation.x + np.random.normal(0, random.uniform(.1, .05), 1))
        msg.pose.pose.orientation.y = np.squeeze(msg.pose.pose.orientation.y + np.random.normal(0, random.uniform(.1, .05), 1))
        msg.pose.pose.orientation.z = np.squeeze(msg.pose.pose.orientation.z + np.random.normal(0, random.uniform(.1, .05), 1))
        msg.pose.pose.orientation.w = np.squeeze(msg.pose.pose.orientation.w + np.random.normal(0, random.uniform(.1, .05), 1))

        msg.twist.twist.linear.x = np.squeeze(msg.twist.twist.linear.x + np.random.normal(0, random.uniform(.1, .05), 1))
        msg.twist.twist.linear.y = np.squeeze(msg.twist.twist.linear.y + np.random.normal(0, random.uniform(.1, .05), 1))
        msg.twist.twist.linear.z = np.squeeze(msg.twist.twist.linear.z + np.random.normal(0, random.uniform(.1, .05), 1))

        msg.twist.twist.angular.x = np.squeeze(msg.twist.twist.angular.x + np.random.normal(0, random.uniform(.1, .05), 1))
        msg.twist.twist.angular.y = np.squeeze(msg.twist.twist.angular.y + np.random.normal(0, random.uniform(.1, .05), 1))
        msg.twist.twist.angular.z = np.squeeze(msg.twist.twist.angular.z + np.random.normal(0, random.uniform(.1, .05), 1))
